...
### Q9：ESP32 有哪些低功耗模式？和 STM32 有什么不同？

**短答：**

ESP-IDF 官方 `Sleep Modes` 里，ESP32 的系统睡眠模式主要是 `Light-sleep` 和 `Deep-sleep`。`Modem-sleep` 不是这个页面里的系统睡眠大类，而是 Wi-Fi/BT 射频省电机制。
...
```text
系统睡眠模式：Light-sleep / Deep-sleep
无线低功耗机制：Modem-sleep / Wi-Fi power save
```
...
它适合短时间空闲、等待 GPIO / 定时器 / UART 等唤醒的场景。
...
大部分 RAM 断电
APB_CLK 驱动的数字外设断电
RTC 控制器、ULP、RTC memory 等低功耗域保留
唤醒后类似重新启动
```

它适合长时间待机、定时采集、按键唤醒、电池设备深度休眠。
...
CPU 仍可运行
Wi-Fi 连接可以保持
Wi-Fi PHY / RF 在空闲周期进入省电
到 Beacon / DTIM 周期再唤醒接收数据
```

所以它更像 Wi-Fi 模块的省电策略，而不是 `esp_light_sleep_start()` 或 `esp_deep_sleep_start()` 这种系统睡眠入口。

ESP32 和 STM32 的区别：

```text
STM32 重点是 MCU、外设、时钟、电源域低功耗。
ESP32 除了 MCU 外，还要重点管理 Wi-Fi/BT modem、RF、RTC 域和 ULP。
```

所以 ESP32 低功耗不能只说“关闭外设”，更准确是：
...
关 APB 外设
保留 RTC 域
根据场景管理 Wi-Fi/BT modem
```

**结合我的项目：**

如果是 ESP32 音频 / 联网项目，不能简单把所有东西都 Deep-sleep。比如语音唤醒、WebSocket 长连接、TTS 播放、Wi-Fi 保活的需求不同：

```text
需要保持实时连接：优先考虑 Wi-Fi power save / Modem-sleep。
短时间空闲等待事件：可以考虑 Light-sleep。
长时间无人使用：可以考虑 Deep-sleep，唤醒后重新初始化网络和业务。
```
...
面试官可能会问：为什么不能一直 Deep-sleep？

可以回答：

> Deep-sleep 功耗最低，但唤醒后更接近重新启动，Wi-Fi 要重新连接，业务状态也要恢复。如果设备需要保持连接或快速响应，就不能只看最低功耗，还要看唤醒延迟、连接恢复时间和用户体验。
...
- 底层 driver 直接操作业务状态。
- 组件拆很多，但依赖互相交叉。
- 把 `Modem-sleep` 和 `Light-sleep / Deep-sleep` 混成同一种概念。
- 只说“关闭外设省电”，没有区分 CPU、RAM、RTC、Wi-Fi/BT modem 和唤醒源。
...
- [ESP-IDF 编程指南：睡眠模式](https://docs.espressif.com/projects/esp-idf/zh_CN/stable/esp32/api-reference/system/sleep_modes.html)
- [ESP-IDF 编程指南：Wi-Fi 场景下低功耗模式](https://docs.espressif.com/projects/esp-idf/zh_CN/stable/esp32/api-guides/low-power-mode/low-power-mode-wifi.html)
