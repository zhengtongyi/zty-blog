...
前半部分先讲 ESP32-S3 ADC 的通用原理；后半部分用 `ESP32-S3-RLCD-4.2` 示例板的电池采样链路说明 PowerService 为什么可以只暴露 `valid / voltage_mv / percent` 这类电量 snapshot 字段。当前板卡充电状态由 Type-C 口附近的 `CHG` LED 人眼判断，固件默认不读取充电状态。
