---
title: 01 问题背景与任务调研
description: 说明为什么要把音频流可靠传输作为独立模块，梳理主要问题、关键任务、指标和方案取舍。
---

## 一句话定位

音频流可靠传输模块负责把上行 PCM 和下行 PCM 稳定送到云端和播放器，解决的不是“能不能发”，而是“在弱网、慢云端、打断、重连和资源争用下，链路还能不能持续正确工作”。
...
> 现在产品已经能跑通语音闭环，但一到真实网络环境就容易暴露出问题。我要的不是再写一个“能发送音频”的函数，而是一个能在弱网、长会话、多轮对话和打断场景里稳定工作的音频流传输模块。它要对 `Session`、`SRService`、`TTSPlayer` 和云端都提供稳定接口，还要能把问题说清楚、量清楚、测清楚。
...
当前这篇只是第一章，定位是问题背景和任务调研。它还不能证明模块已经完成优化，也不能证明链路已经达到长稳要求。
