...
设备侧按 `Application -> Service -> BSP / ESP-IDF driver -> Hardware` 理解更贴近当前仓库。`Driver/HAL` 仍然是一个有用的概念 seam，但当前没有独立完整的 Driver 目录层；简单外设的 driver 适配已经被 Service 或 BSP 吸收。
...
ESP-IDF / third-party driver
  adc_oneshot / adc_cali / gpio / i2s / iot_button
...
- `Driver/HAL` 当前不是独立目录层，而是 ESP-IDF/第三方 driver API 或被 Service/BSP 包住的底层适配。
...
### 2. 为什么要分 Application / Service / BSP？Driver 边界在哪里？
...
当前仓库里 Driver 不是独立完整一层，而是以两种形式存在：一种是 ESP-IDF/第三方 driver API，例如 `adc_oneshot_*`、`adc_cali_*`、`gpio_*`、`i2s_*`、`iot_button_*`；另一种是被 BSP 或 Service 包住的底层适配，例如 I2C/I2S 总线由 BSP 收口，PowerService 直接调用 ADC oneshot/calibration。只有当同一个芯片被多处复用、协议复杂或需要替换硬件时，才值得抽独立 Driver adapter。
...
- 能画出当前实际分层：Application -> Service -> BSP / ESP-IDF driver -> Hardware。
- 能说明 Driver 边界仍存在，但当前不是独立完整目录层。
